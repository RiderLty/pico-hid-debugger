...
static volatile uint32_t sof_frame_number;
...
uint32_t pio_usb_host_get_frame_number(void) {
  return sof_frame_number;
}

bool pio_usb_host_endpoint_abort_transfer(uint8_t root_idx, uint8_t dev_addr, uint8_t ep_addr) {
  (void)root_idx;
  (void)dev_addr;
  (void)ep_addr;
  // not implemented
