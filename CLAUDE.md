...
| `index.html` | Web Serial 日志查看器：**xterm.js + WebGL 渲染**（vendor/ 于 `vendor/`，UMD 挂载注意：xterm 展开式、fit/webgl 命名空间式），默认 2M，**固件输出开关勾选框**（`data-sw` 位号须与 `src/log_switch.h` 一致；localStorage `switch` 记忆，默认 `0x13`，连接打开即下发、`[BOOT]` 行补发 + **按需清空上一轮日志**（本机偏好勾选框「重启清空」，localStorage `clearOnBoot` 默认开：收到 `[BOOT]` 行时先 `clearLog()` 再补发掩码，清空与「清空日志」按钮共用一条路径，含 `pending`/`lineBuf`/`term.reset()`），下发经 promise 链串行化——`WritableStream` 同一时刻只能有一个 writer）。勾选框**一个控件管两件事**：下发掩码 + 本地按位显示（`TAG_INFO` 表同时给出配色与开关位，两者不是一回事：`[BOOT]` 与 `[MOUNT]` 同色但前者恒开）。按位显示不是冗余——固件默认全开，而 `[BOOT]` 先于 core1 打印，队列会把启动那波 `[TUSB]` 缓冲在后，掩码翻回来还要再吐一段。正则内容过滤（忽略大小写，历史存 localStorage regexHist/regex，input 防抖 400ms 实时应用、回车/失焦记忆，无效红框保持上次视图），贴底跟随为 xterm 原生语义（视口 scroll 判贴底 + 回到底部角标），授权持久化 + `connect` 事件 + 100ms 轮询看门狗自动重连，ANSI 着色，模型上限 50000 行 |
...
6. **PIO-USB 版本策略（重要）**：0.6.0 的重写带来低速支持、但破坏了 hub 上设备拔出（上游 issue #149 / TinyUSB #2971 报告人 bisect 到那对提交；上游自己认定最后一个能正确处理的是 `0f747aa`）。本仓库**固定旧血脉顶端 `9510f79` 并放弃低速支持**：`CMakeLists.txt` 会检查子模块仍是旧血脉（缺 `pio_usb_host_task` 即 `FATAL_ERROR`），`build.sh`/cmake 还会确保 `patches/pio_usb/` 里的三枚补丁在位；`[BOOT]` 的 `piousb=` 用于核对实际刷入的提交。**另有一处与版本无关的补丁**：SDK 捆绑的 TinyUSB 0.18.0 hub 驱动一次传输失败就永久停摆（上游 PR #2994 / 0.19.0 才修好），由 configure 期生成 `hub.c` 副本补上（未打时症状：`hub_port_get_status_complete ... ASSERT FAILED` 后 hub 事件彻底断绝）。针对 0.6+ 血脉写的 9 枚补丁与整轮调查结论见 `patches/pio_usb_archive_0.6plus/`。
...
8. **主机冻结 >3ms 救不回 hub 后面总线供电的 Xbox 手柄（软件侧已穷举）**：补丁 `0004-host-stall-recover` 的 root 口软插拔靠"重新枚举的总线复位"唤醒 suspend 设备。实测结论（当时的冻结扫描探针 + `[TUSB]` 逐帧日志，探针已随调查结束移除）：
   - 失联门限 **≈3ms**（250us 步进下 2750us 存活 / 3000us 失联），与 USB 规范"设备 3ms 无总线活动即 suspend"一致；把自愈关掉（`-DPIO_USB_STALL_RECOVER_US=0`）则设备**永不回来**（EP0 与控制传输一直 FAILED）——说明自愈本身是必需的。
   - **HID 设备**（无会话状态）自愈后正常恢复：重新枚举、`[MOUNT]`/`[DEVDS]` 完整、报文流继续。
   - **Xbox One/Series（GIP，实测 045e:02ea 经 WCH hub 1a86:8091）**：重新枚举成功、GIP 握手 OUT 全部被 ACK（`on EP 02 … OK`），但**输入管道不重启** —— `on EP 82 … FAILED`（对端不应答）、挂载之后再无任何手柄报文，只有 **VBUS 掉电（物理拔插）** 能恢复。已穷举的软件手段全部无效：重放初始化握手、`GIP_PWR_RESET`、`GIP_PWR_OFF`、额外输入包请求。
   - 未走的路（按价值）：① 让 hub 给下游端口掉电上电（等价物理拔插，需 hub 支持，这台声明的是 ganged power switching）；② 把 SOF 生成交给 PIO 硬件（核心冻结时不 suspend，从根上解决）；③ 认真实现一次 USB resume（规范正解，不需要复位）。
